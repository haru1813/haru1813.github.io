  ...
  "posts": [
    ...
    },
    {
      ...
      "content": [
        ...
        "JPA는 **표준**이라서 “JPA만 설치”해선 실제로 동작하지 않습니다. 구현체가 필요하고, Spring Boot는 기본적으로 Hibernate를 많이 사용합니다.",
        "",
        "## 핵심 개념 4가지",
        ...
        "- 복잡한 조회는 엔티티가 아니라 **DTO/전용 쿼리**로 분리하는 것도 전략",
        "- “JPA를 쓴다” = “SQL을 몰라도 된다”가 절대 아님",
        "",
        "---",
        "### 요약",
        "Spring Boot에서 JPA를 사용한다는 말은 보통 **Hibernate를 통해 JPA 표준을 사용한다**는 뜻입니다.",
        "그리고 성능/버그의 대부분은 **트랜잭션, 지연 로딩, 쿼리**에서 생기니 여기를 중심으로 공부하면 좋습니다."
      ]
    },
    {
      ...
      "content": [
        "## 디자인 패턴이란?",
        "디자인 패턴은 소프트웨어 설계에서 자주 반복되는 문제를 해결하는 **재사용 가능한 설계 아이디어**입니다.",
        "중요한 포인트는 ‘코드 조각’이 아니라 **구조와 역할**이라는 점이에요.",
        "",
        "## 왜 패턴을 배울까",
        ...
        "",
        "## 패턴을 쓸 때의 주의점",
        "- 패턴은 **도구**입니다. 문제보다 패턴이 먼저 나오면 과설계가 됩니다.",
        "- “패턴 적용” 자체가 목표가 아니라, **변경에 강한 구조**가 목표입니다.",
        "",
        "## 대표 분류(GoF: 생성/구조/행위)",
        "### 1) 생성(Creational) 패턴",
        "객체를 ‘어떻게 만들지’를 다룹니다.",
        ...
        "---",
        "### 요약",
        "디자인 패턴은 ‘외우는 목록’이 아니라 **변경에 강한 구조를 만드는 사고 도구**입니다.",
    ...
    },
    {
      ...
      "content": [
        "## Factory Method란?",
        "**Factory Method 패턴**은 ‘객체를 생성하는 책임’을 별도의 메서드(팩토리 메서드)로 분리해서,",
        "클라이언트가 구체 클래스를 직접 `new` 하지 않도록 만드는 생성 패턴입니다.",
        "",
        "핵심은 이거예요:",
        "- **사용하는 쪽**은 인터페이스(상위 타입)만 안다.",
        "- **무엇을 생성할지**는 Creator(또는 그 서브클래스)가 결정한다.",
        "",
        "## 언제 필요할까",
        ...
        "",
        "## 정리",
        "- Factory Method는 **‘new를 숨기는 패턴’**이 아니라, **생성 책임을 분리해 변경에 강한 구조**를 만드는 방법입니다.",
    ...
    },
    {
      ...
      "content": [
        ...
        "",
        "## 머신러닝(ML)",
        "머신러닝은 크게 **지도학습 / 비지도학습 / 강화학습**으로 나눠서 생각하면 편합니다.",
        "",
    ...
    },
    {
      ...
      "date": "2026-02-10",
      "summary": "프롬프트 엔지니어링은 ‘말을 예쁘게 쓰는 기술’이 아니라, 모델이 올바른 출력에 도달하도록 **목표·맥락·제약·검증**을 설계하는 작업입니다.",
      "category": "Technology/AI",
      "tags": ["ai", "prompt", "llm", "basics"],
      "content": [
        "## 프롬프트 엔지니어링이란?",
        "LLM(대규모 언어 모델)에게 원하는 결과를 얻기 위해 **질문(프롬프트)**을 구조화하고,",
        "**맥락/제약/출력 형식/평가 기준**을 명확히 설계하는 방법입니다.",
        "",
        "## 왜 중요한가",
        "- 같은 모델이라도 프롬프트에 따라 결과 품질이 크게 달라집니다.",
        "- ‘정답’이 아니라도, **일관성/재현성**이 필요합니다.",
        "- 특히 실무에서는 “좋은 답”보다 **안전하고 예측 가능한 답**이 중요할 때가 많습니다.",
        "",
        "## 좋은 프롬프트의 기본 구성(템플릿)",
        "- **역할(Role)**: 너는 무엇을 하는가?",
        ...
        "---",
        "### 요약",
        "프롬프트 엔지니어링의 핵심은 **명확한 목표와 제약, 그리고 평가 기준**입니다.",
